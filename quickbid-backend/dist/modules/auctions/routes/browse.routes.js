...
exports.auctionsBrowseRouter = void 0;
const express_1 = __importDefault(require("express"));
const supabaseAdmin_1 = require("../../../supabaseAdmin");
const pixelEvent_model_1 = require("../../marketing/models/pixelEvent.model");
exports.auctionsBrowseRouter = express_1.default.Router();
function mapAuctionStatus(dbStatus, startDate, endDate, winnerId, finalPrice) {
    ...
    if (s === 'live' || s === 'active') {
        if (endsAt && endsAt > now)
            return 'live';
        if (endsAt && endsAt <= now)
            return winnerId ? 'sold' : 'expired';
        return 'live';
    }
    if (s === 'scheduled' || (startsAt && startsAt > now))
        return 'upcoming';
    if (s === 'ended')
        return winnerId || (finalPrice != null && finalPrice > 0) ? 'sold' : 'expired';
    return 'upcoming';
}
exports.auctionsBrowseRouter.get('/auctions/detail', async (req, res, next) => {
    try {
        ...
        let auction = null;
        const { data: activeAuction } = await supabaseAdmin_1.supabaseAdmin
            .from('auctions')
        ...
        if (activeAuction) {
            ...
            const { data: latestAuction } = await supabaseAdmin_1.supabaseAdmin
                .from('auctions')
                .select('id, status, start_date, end_date, current_price, reserve_price, winner_id, final_price')
                .eq('product_id', product_id)
        ...
        let watchersCount = 0;
        const { data: wishRows } = await supabaseAdmin_1.supabaseAdmin
            .from('wishlist')
            .select('product_id')
            .eq('product_id', product_id);
        ...
        let bids = [];
        if (auction?.id) {
            const { data: bidRows } = await supabaseAdmin_1.supabaseAdmin
                .from('bids')
                .select(`
          *,
        ...
        let viewerCount = 0;
        try {
            viewerCount = await pixelEvent_model_1.PixelEventModel.countDocuments({
                event: 'auction_view',
                ts: { $gte: oneHourAgo },
        ...
        if (auction?.id) {
            try {
                const { count: recentBidsCount, error: recentErr } = await supabaseAdmin_1.supabaseAdmin
                    .from('bids')
...
exports.auctionsBrowseRouter.get('/auctions/browse', async (req, res, next) => {
    try {
        if (!supabaseAdmin_1.supabaseAdmin) {
            return res.status(500).json({ error: 'Supabase admin not configured' });
        }
        const { category, auction_type, status, product_id, price_min, price_max, location_query, brand, sort_by, show_trending, show_premium, show_featured, page = '1', page_size = '20', } = req.query;
        ...
        if (productIds.length > 0) {
            let productsQuery = supabaseAdmin_1.supabaseAdmin
                .from('products')
        ...
        if (auctionIds.length > 0) {
            ...
            if (!bidsErr && Array.isArray(bids)) {
                for (const row of bids) {
                    const id = row.auction_id;
                    bidCounts[id] = (bidCounts[id] || 0) + 1;
                }
            }
        }
        // Aggregate watchers per product for current page
        const watchersCounts = {};
        if (productIds.length > 0) {
            const { data: wishRows, error: wishErr } = await supabaseAdmin_1.supabaseAdmin
                .from('wishlist')
                .select('product_id')
        ...
        let items = (auctions || []).map((a) => {
            ...
            return {
                ...
                watchers_count: watchersCounts[a.product_id] || 0,
                make: p.make || null,
        ...
        if (wantTrending)
            items = items.filter((it) => Boolean(it.is_trending));
        if (wantPremium)
            items = items.filter((it) => Boolean(it.is_premium));
        if (wantFeatured)
            items = items.filter((it) => Boolean(it.is_featured));
        if (sort_by === 'newest') {
