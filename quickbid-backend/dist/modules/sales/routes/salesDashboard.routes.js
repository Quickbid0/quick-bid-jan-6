...
exports.salesRouter.get('/sales/dashboard', authMiddleware_1.requireAuth, async (req, res, next) => {
    try {
        ...
        const totalGMV = auctions.reduce((sum, a) => sum + (a.final_price || 0), 0);
        const totalNetPayout = payouts.reduce((sum, p) => sum + (p.net_payout || 0), 0);
        const newSellersCount = newSellers.length;
        // Geo stats by city (best-effort from products.location)
        const { data: geoRows } = await supabaseAdmin_1.supabaseAdmin
            .from('products')
            .select('id, location')
        ...
        auctions.forEach((a) => {
            const product = geoMapSource.find((p) => p.id === a.id);
            if (!product)
                return;
        ...
        const totalPaidSaleValue = payouts.reduce((sum, p) => sum + (p.sale_price ? Number(p.sale_price) : 0), 0);
        ...
        if (ownerIds.length > 0) {
            const { data: profiles } = await supabaseAdmin_1.supabaseAdmin
                .from('profiles')
...
exports.salesRouter.get('/sales/leads', authMiddleware_1.requireAuth, async (req, res, next) => {
    try {
        if (!supabaseAdmin_1.supabaseAdmin) {
            return res.status(500).json({ error: 'Supabase admin not configured' });
        }
        ...
        if (search) {
            const term = `%${search}%`;
            query = query.or(`name.ilike.${term},company.ilike.${term},city.ilike.${term},state.ilike.${term}`);
        }
...
// Create new lead
exports.salesRouter.post('/sales/leads', authMiddleware_1.requireAuth, async (req, res, next) => {
    try {
        if (!supabaseAdmin_1.supabaseAdmin) {
            return res.status(500).json({ error: 'Supabase admin not configured' });
        }
        const { source = 'seller', name, phone, email, city, state, company, owner_user_id, status = 'new', value_gmv, notes, next_follow_up_at, } = req.body || {};
...
exports.salesRouter.patch('/sales/leads/:id', authMiddleware_1.requireAuth, async (req, res, next) => {
    try {
        if (!supabaseAdmin_1.supabaseAdmin) {
            return res.status(500).json({ error: 'Supabase admin not configured' });
        }
