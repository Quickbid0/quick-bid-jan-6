...
async function seed() {
    ...
    for (const role of roles) {
        const doc = await Role_1.Role.findOneAndUpdate({ key: role.key }, { $set: role }, { upsert: true, new: true }).lean();
    ...
    for (const perm of permissions) {
        const doc = await Permission_1.Permission.findOneAndUpdate({ key: perm.key }, { $set: perm }, { upsert: true, new: true }).lean();
    ...
    for (const m of mappings) {
        ...
        if (!role || !perm)
            continue;
        await RolePermission_1.RolePermission.findOneAndUpdate({ roleId: role._id, permissionId: perm._id }, { $setOnInsert: { allowed: true } }, { upsert: true, new: true });
    }
