...
export async function runAuctionTick(options: TickOptions = {}): Promise<void> {
  if (!supabaseAdmin) {
  ...
  for (const auction of auctions as any[]) {
    ...
    if (secondsToEnd > 0 && secondsToEnd <= extensionThresholdSeconds && latestBid) {
      ...
      if (events) {
        ...
        try {
          bidding_stats = await computeBiddingStats(auction.id as string);
        } catch (statsErr) {
          console.error('runAuctionTick: computeBiddingStats (extend) error', statsErr);
        }

        events.emitAuctionExtended(auction.id as string, {
    ...
    if (endDate <= now) {
      if (!hasBids) {
        ...
        if (events) {
          ...
          events.emitAuctionFinalized(auction.id as string, {
            auctionId: auction.id,
            winnerId: null,
            finalPrice: null,
            bidding_stats,
      ...
      if (auction.seller_id) {
        try {
          const { data: existingPayout, error: payoutCheckErr } = await supabaseAdmin
            .from('payouts')
            .select('id')
          ...
          if (payoutCheckErr) {
              ...
              if (auction.product_id) {
                ...
                if (productErr) {
                  ...
                  const primaryCategory =
                    (product as any).main_category || (product as any).category || (product as any).sub_category || null;
                  ...
                  if (primaryCategory) {
                    ...
                    if (rulesErr) {
                      ...
                      const mode = rule.mode as 'flat' | 'percent';
                      ...
                      if (mode === 'percent' && percent !== null) {
                        ...
                        if (minCommission !== null && value < minCommission) value = minCommission;
            ...
            const { error: payoutInsertErr } = await supabaseAdmin.from('payouts').insert({
