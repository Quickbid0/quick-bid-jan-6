...
const AdminBranches: React.FC = () => {
  ...
  const [form, setForm] = useState<Partial<Branch>>({ name: '', code: '', state: '', city: '', address: '' });
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForm({ name: '', code: '', state: '', city: '', address: '' });
      await loadBranches();
    } catch (err: any) {
  ...
  const handleToggleActive = async (branch: Branch) => {
    try {
      ...
      if (branch.isActive) {
        ...
        if (!res.ok) throw new Error('Failed to disable branch');
      } else {
        const res = await fetch(`/api/branches/${branch._id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (!res.ok) throw new Error('Failed to enable branch');
      }
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400">
            Manage branches/locations used for winner pickup, staff assignment, and local operations.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <form onSubmit={handleCreate} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 lg:col-span-1">
          <h2 className="text-sm font-semibold mb-3 text-gray-900 dark:text:white">Add Branch</h2>
        ...
        <div className="lg:col-span-2 bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-gray-900 dark:text-white">Existing Branches</h2>
          </div>
          ...
          {!loading && !items.length && !error && (
            <div className="py-6 text-sm text-gray-500">No branches yet. Create the first one on the left.</div>
          )}
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-gray-50 dark:bg-gray-900/60">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-600 dark:text-gray-300">Code</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-600 dark:text-gray-300">City / State</th>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {items.map((b) => (
                  <tr key={b._id}>
                    <td className="px-3 py-2 text-gray-900 dark:text-white">{b.name}</td>
                    <td className="px-3 py-2 text-xs text-gray-500">{b.code}</td>
